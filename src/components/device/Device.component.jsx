...
export default class Device extends Component {
                 constructor() {
                   super();
                   this.state = { activeComponent: 'Update Device', selection: 'Update Device', endpoint: '/users/v2', method: null, showToaster: true, companies: [], value: '', loading: true, devices: [], name: null, policy_id: null, add_zone_ids: '', remove_zone_ids: '' };
                 }

                 data = [{ key: 'PUT-device', value: 'Update Device', text: 'Update Device' }, { key: 'POST-device', value: 'Get Devices', text: 'Get Devices' }, { key: 'GET-device', value: 'Get Device', text: 'Get Device' }, { key: 'GET-device-threats', value: 'Get Device Threats', text: 'Get Device Threats' }, { key: 'GET-device-zone', value: 'Get Zone Devices', text: 'Get Zone Devices' }, { key: 'DELETE-devices', value: 'Delete Devices', text: 'Delete Devices' }, { key: 'GET-device-MAC', value: 'Get By MAC Address', text: 'Get By MAC Address' }];
                 ...
                 componentDidMount() {
                   axios
                     ...
                     .then(res => {
                       this.setState({
                         loading: false,
                         companies: res.data.data.companies.map(
                           company => {
                             return {
                               value: company.name,
                               text: company.name
                 ...
                 updateDevice_ = () => {
                   let { name, policy_id, add_zone_ids, remove_zone_ids, device_id } = this.state;

                   let data = { name, policy_id };

                   let url_ = `${config.API_BASE_URL}single-device/${device_id}?comp≤any_name=${this.state.value}`;
                   ...
                   switch (this.state.activeComponent) {
                     case 'Update Device':
                       return <div>
                           ...
                           <UpdateDevice value={this.state.value} handleInputChange={this.handleInputChange} />
                     ...
                     case 'Get Devices':
                       return <div>
                           ...
                           {this.state.devices.length === 0 ? <LoaderGraphic /> : <Fragment>
                     ...
                     case 'Get Device':
                       return <div>
                           ...
                           <GetDevice value={this.state.value} getDevices={this.state.devices} />
                         </div>;

                     case 'Get Device Threats':
                       return <div>
                           <SubHeader info="Allows a caller to request a page with a list of device resources belonging to a Tenant," />
                           <GetDeviceThreats value={this.state.value} getThreatDevice={this.state.devices} />
                         </div>;
                     case 'Get Zone Devices':
                       return <div>
                     ...
                     case 'Get By MAC Address':
                       return <div>
                           <SubHeader info="Allows a caller to request a page with a list of device resources belonging to a Tenant," />
                           <GetByMACAddress value={this.state.value} getDevices={this.state.devices} />
                         </div>;
                     case 'Delete Devices':
                       return <div>
                           <SubHeader info="Allows a caller to delete one or more devices from an organization." />
                           <DeleteDevices value={this.state.value} getDevices={this.state.devices} />
                         </div>;
                   }
                 };
                   ...
                   return <div className="device-container">
                       ...
                       <div className="header-nav">
                         <div className="dropdwn-nav">
                           <div>
                             <Dropdown placeholder="Select Company" search selection onChange={(_, { value }) => {
                                 this.setState({
                                   value,
                                   disabled: false
                                 });
                               }} options={this.state.companies} loading={this.state.loading} />
                           </div>
                           <div>
                             <Dropdown placeholder="Update Device" fluid selection options={this.data} onChange={this.handleChange} />
                           </div>
                         </div>
